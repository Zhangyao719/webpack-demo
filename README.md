...
(也可自定义执行webpack文件: `npx webpack --config 自定义webpack文件名`)

> 实际开发中根据不同的开发环境可能会有多个webpack.config.js文件
...
还可以通过配置`webpack.config.js`文件对watch的参数进行修改：
...
2. index.html中修改 `<script src="/bundle.js"></script>`(会自动在内存中创建自己的bundle文件)

3. 运行：`npx webpack-dev-server`

4. 运行：`npx webpack-dev-server --hot --open --port 8090 --compress --contentBase src`

5. 配置`package.json`的scripts：`"dev": "webpack-dev-server --hot --open --port 8090"`

6. 运行`npm run dev`

> "webpack-dev-server"的常见配置:
>
> 1. --port 7000 : 端口设置
> 2. --open         : 自动开启服务器
> 3. --hot            : 开启热更新(只重新编译更新部分)
> 4. --compress :  开启express的gzip压缩
> 5. --contentBase : 文件生成路径(默认在根目录位置生成: localhost:8080/bundle.js)

devServer会`在内存中(不是项目文件中)`生成一个打包好的`bundle.js`，专供开发时使用，打包效率高，修改代码后会自动重新打包以及刷新浏览器，用户体验非常好
...
plugins: [
    new HtmlWebpackPlugin({
        filename: 'index.html',       // 最终在dist目录下打包生成的index.html文件
        template: './src/index.html'  // 生成index.html时参照的临时模板
    })
]
```
...
```js
{
    ...
    use: [{
        ...
        options: {
            ...
            limit: 5 * 1024,
            // 打包输出目录(直接写目录名, 会跟在打包的根目录下生成二级目录)
            outputPath: 'images',
            // 打包输出图片名称(文件名+根据图片名生成的前四位哈希值.扩展名)
...
2. 如果需要支持更高级的ES6语法, 可以继续安装插件:
   ...
   `npm i @babel/plugin-proposal-class-properties -D`  (这里安装的是转译class相关的语法)
...
// webpack.config.js
{
  ...
  use: {
    loader: 'babel-loader',
    options: {
      presets: ['@babel/env'], // 预设 能够转换大部分es6的语法了
      plugins: [
          '@babel/plugin-proposal-class-properties']
    }
  },
  exclude: /node_modules/  // 该文件夹下的js不需要转译, 排除掉
  include: path.(__dirname, '../src') // 只在src文件夹内使用babel转译(推荐)
}
```
...
如果需要使用`generator(es6函数生成器)`，无法直接使用babel进行转换，因为会将`generator`转换为一个`regeneratorRuntime`，然后使用`mark`和`wrap`来实现`generator`
...
```json
{
  ...
  "plugins": [
    "@babel/plugin-proposal-class-properties", // class的一些属性需要用到的转译插件
    "@babel/plugin-transform-runtime" // generator函数生成器需要用到的转译插件
  ]
}
```
...
如果需要使用ES6/7中对象原型提供的新方法，babel默认情况无法转换，即使用了`transform-runtime`的插件也不支持转换原型上的方法, 因为js是动态语言, 在执行代码的时候随时为对象添加属性或方法, 在没有调用该属性(比如数组方法)时, babel不知道其原型上是否有这种属性, 所以不会转译.
...
### source map[的使用](https://www.webpackjs.com/guides/development/#%E4%BD%BF%E7%94%A8-source-map)

> 当 webpack 打包源代码时，可能会很难追踪到错误和警告在源代码中的原始位置。 [source map](http://blog.teamtreehouse.com/introduction-source-maps) 功能，将编译后的代码映射回原始源代码
...
该插件在`npm run build`时自动清除上一次打包的`dist`目录, 重新生成新的`dist`，非常方便
...
> 将单个文件或整个目录复制到构建目录. 比如将静态资源(图片/视频/音频)拷贝一份放入构建目录dist文件夹中(不参与打包)
...
> 对HTML里面的所有<img>引入的图片进行打包

1. 安装`npm install -D html-withimg-loader`
...
1. 在`webpack.config.js`中修改入口和出口配置
   ...
   ```js
     ...
     output: {
       path: path.join(__dirname, './dist/'),
       // filename: 'bundle.js',
       // 2. 多入口无法对应一个固定的出口, 所以修改filename为[name]变量,打包两个对应的js
       filename: '[name].js',
       publicPath: '/'
     },
     plugins: [
         ...
         new HtmlWebpackPlugin({
             template: './index.html',
             filename: 'index.html',
             chunks: ['main'] // 指定引入main,js, 多个以","分开
         }),
         new HtmlWebpackPlugin({
             template: './index.html',
             filename: 'other.html',
             // chunks: ['other', 'main']
             chunks: ['other'] // // 指定引入other.js
         })
     ]
   ```
...
- expose-loader **将库引入到全局作用域**
  ...
  2. 配置loader
     ...
     tips: `require.resolve` 用来**获取模块的绝对路径**。所以这里的loader只会作用于 jquery 模块。并且只在 bundle 中使用到它时，才进行处理。
...
2. 然后在dev和prod中使用`webpack-merge`把自己的配置与base的配置进行合并后导出
   ...
   ```tsx
   // 1.在webpack.dev.js中 导入webpack.base.js
   const merge = require('webpack-merge');
   const baseConfig = require('./webpack.base.js')
   
   // 2. 使用merge方法 生成一个对象, 最终让module.exports导出
   // merge(参数1:导入的文件, 参数2: 当前配置)
...
3. 将package.json中的脚本参数进行修改，通过`--config`手动指定特定的配置文件
   ...
   ```tsx
   // package.json中根据不同环境分别配置不同的命令
   "scripts": {
       "dev": "webpack-dev-server --config ./webpack.dev.js", // 指定开发环境下的webpack文件
       "build": "webpack --config ./webpack.prod.js", // 打包时指定生产模式下的webpack文件
   }
   ```
...
module.exports = merge(baseConfig, {
	plugins: [
        new webpack.DefinePlugin({
          IS_DEV: 'false' //此处写字符串,会自动解析成false
        })
    ],
})
```
...
```tsx
// 自动根据不同环境切换host接口
let host = 'http://dev.wetest.com'  (let host = process.env.BASE_URL)
if(!IS_DEV) {
    host = 'http://pre.wetest.com'
}
```
...
```tsx
// 1. 在.env.dev里配置开发环境的各个请求接口:
{
    "WT_RUN_ENV": "local",
    "WT_UPLOAD_URL": "http://9.134.119.196:8081",
    "WT_BASE_GO_API_URL": "http://9.134.244.12:8081/v1",
}
    
// 2. 在http.js中通过process.env.REACT_APP_XXX获取到配置的环境变量, 配置axios请求方法:
import axios form 'axios'; 
    
const BASE_URL = process.env.WT_BASE_GO_API_URL;
...
export const http = axios.create({
    ...
    headers: {					//配置请求头
        'Content-Type': 'application/json', //post请求的数据格式
...
3. 创建插件对象，配置抽离的css文件名，支持placeholder语法
   ...
   new MiniCssExtractPlugin({
   	filename: '[name].css' // 输出的文件名为入口文件的文件名
   })
   ```
...
3. 项目**根目录**下添加`postcss`的配置文件：`postcss.config.js` 或者`'.postcssrc.js'`
...
4. 在`postcss`的配置文件中使用插件
   ...
   // postcss.config.js
   module.exports = {
     plugins: [require('autoprefixer')] // 配合使用autoprefixer插件
   }
   ```
   ...
   plugins: [
       new MiniCssExtractPlugin({
           filename: '[name].css', // webpack.client.cong.js: '[name].[hash].css'
           ignoreOrder: true, // 禁用顺序检查 ( 这对 CSS 模块很有用！)
       }),
   ]
   //.postcssrc.js
   module.exports = {
       "plugins": {
           "postcss-import": {},
           "postcss-url": {},
           "autoprefixer": {} // 使用了autoprefixer插件
       }
   }
   ```
...
Code Splitting是webpack打包时用到的重要的优化特性之一，此特性能够**把JS代码分离到不同的 bundle** 中，然后可以按需加载或并行加载这些文件。代码分离可以用于获取更小的 bundle，以及控制资源加载优先级，如果使用合理，会极大影响加载时间。

有三种常用的代码分离方法：

- 入口起点(entry points)(不推荐)：使用`entry`配置手动地分离代码。
- 防止重复(prevent duplication)：使用 `SplitChunksPlugin`去重和分离 chunk。
- **`动态导入(dynamic imports)(常用)`**：通过模块的内联函数调用来分离代码。
...
2. 情景再现: 在main.js和other.js中都引入同一个模块，并使用其功能
...
3. 修改package.json的脚本，添加一个使用dev配置文件进行打包的脚本（目的是不压缩代码时查看打包的bundle更方便）
...
4. 查看`vendors~main~other.bundle.js`，其实就是把都用到的公共部分jQuery打包到了一个单独的js中
...
2. 修改.babelrc配置文件，添加`@babel/plugin-syntax-dynamic-import`插件
   ...
   ```json
   {
     ...
     "plugins": [
       ...
       "@babel/plugin-syntax-dynamic-import" // 添加允许inport动态导入的插件
     ]
   }
   ```

3. 案例: 将jQuery模块进行动态导入

   ```js
   function getComponent() {
     // 此时可以在任何位置使用import
     return import('jquery').then(({ default: $ }) => { // 修改默认导入模块的名字为$
       return $('<div></div>').html('main')
     })
   }
   
   // tips: import('jquery') 此时返回的就是promise对象
   // 不支持promise的 可以添加:
   // src/main.js
...
可以在webpack配置文件的`module`节点下加上`noParse`，并配置正则来**确定不需要解析依赖关系的模块**
